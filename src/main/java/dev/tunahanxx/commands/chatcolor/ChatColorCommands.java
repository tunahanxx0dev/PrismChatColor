package dev.tunahanxx.commands.chatcolor;
...
public class ChatColorCommands implements TabExecutor {

    private static final StringWrap stringWrap = new StringWrap();
    private static final ChatColorGUI chatColorGUI = new ChatColorGUI();
    ...
    public boolean onCommand(@NotNull CommandSender s, @NotNull Command c, @NotNull String l, @NotNull String @NotNull [] a) {
        ...
        List<String> helpText = mfcConfig.getStringList("messages.default.help");

        if (!(s instanceof Player player)) {
            s.sendMessage(stringWrap.onlyInGameUsage());
            return true;
        }

        if (!player.hasPermission("prismchatcolor.chatcolor")) {
            player.sendMessage(stringWrap.noPermission());
